...
(defun org-transclusion--is-link-to-add-p ()
  "Return non-nil if the link is to be transcluded.
It is meant to be called by `org-transclusion-add-all-in-buffer'.

Assume the point is always at the beginning of a link."

  (save-excursion
    (unless (eq (line-beginning-position)(point)) ;; cannot be the beginning of a line
      (forward-char -1)
         ...
         (save-excursion
           (save-restriction
             ...
             (when (org-element-link-parser)
               (when (org-transclusion--is-link-to-add-p)
                 (org-open-at-point)))
             (while (eq t (org-next-link))
               ;; Check if the link is in the beginning of a line
               ;; Check if the link at point is NOT within tranclusion
               (when (and (org-transclusion--is-link-to-add-p)
